...
interface ReservationJpaRepository : JpaRepository<Reservation, Long> {
    ...
    fun findByIdWithSlotAndProduct(
        @Param("id") id: Long,
    ): Reservation?
    ...
    @Query(
        "select r.id from Reservation r where r.status = :status and r.expiresAt is not null and r.expiresAt <= :now order by r.expiresAt asc",
    )
    ...
    fun findExpiringSoonPendingIds(
        @Param("threshold") threshold: LocalDateTime,
        @Param("now") now: LocalDateTime,
    ): List<Long>

    @Query("select count(r) > 0 from Reservation r where r.reservationSlot.id = :reservationSlotId")
    fun existsByReservationSlotId(
        @Param("reservationSlotId") reservationSlotId: Long,
    ): Boolean
