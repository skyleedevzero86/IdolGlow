...
class PlatformAuthController(
    ...
    ): ResponseEntity<ApiResponse<JwtToken>> = ResponseEntity.ok(ApiResponse.success(authenticationService.login(request)))

    @Operation(summary = "토큰 갱신")
    @PostMapping("/refresh")
    fun refresh(
        @Valid @RequestBody request: RefreshTokenRequest,
    ): ResponseEntity<ApiResponse<JwtToken>> = ResponseEntity.ok(ApiResponse.success(authenticationService.refresh(request)))

    @Operation(summary = "회원가입", description = "가입 즉시 사용 가능(계정 상태 승인)")
    @PostMapping("/register")
    fun register(
        @Valid @RequestBody request: UserRegistrationRequest,
    ): ResponseEntity<ApiResponse<PlatformUser>> = ResponseEntity.ok(ApiResponse.success(authenticationService.register(request)))
