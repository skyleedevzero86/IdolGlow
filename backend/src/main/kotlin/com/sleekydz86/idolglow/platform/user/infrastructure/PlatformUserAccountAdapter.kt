...
class PlatformUserAccountAdapter(
    ...
    override fun save(platformUser: PlatformUser): PlatformUser {
        val entity =
            platformUser.id?.takeIf { it > 0 }?.let { userJpaRepository.findById(it).orElse(null) }
    ...
    override fun findById(id: Long): Optional<PlatformUser> = Optional.ofNullable(userRepository.findById(id)?.toPlatformUser())
    ...
    override fun existsByEmail(email: String): Boolean = userRepository.findByEmail(email) != null

    override fun findByEmailAndUsername(
        email: String,
        username: String,
    ): Optional<PlatformUser> {
        val user =
            userJpaRepository.findByEmailAndPlatformUsername(email, username)
